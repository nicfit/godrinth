...
func TestFacetAnd(t *testing.T) {
	f1 := MakeFacet("project_type", OperandTypeEqual, "mod")
	f2 := MakeFacet("version", OperandTypeGreaterThanOrEqual, "1.20")
	and := MakeFacetAnd(f1, f2)
	fmt.Println(and.String())
	if and.String() != "[[\"project_type=mod\"],[\"version>=1.20\"]]" {
		t.Fail()
	}
}

func TestFacetComplex(t *testing.T) {
	versions := MakeFacetOr(MakeFacet("version", OperandTypeEqual, "1.20.1"),
		MakeFacet("version", OperandTypeGreaterThanOrEqual, "1.20.4"))
	types := MakeFacetAnd(MakeFacet("pr"))
